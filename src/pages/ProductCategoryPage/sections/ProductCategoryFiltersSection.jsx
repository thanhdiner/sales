...
const QUICK_FILTERS = [
  { label: 'Giao hàng tự động', value: 'auto-delivery' },
  { label: 'Tài khoản bản quyền', value: 'licensed' },
  { label: 'Hỗ trợ 24/7', value: 'support' },
  { label: 'Sản phẩm nổi bật', value: 'featured' }
]

function ProductCategoryFiltersSection({ searchInput, sortBy, selectedFeatures, onSearchChange, onSortChange, onFeatureFiltersChange }) {
  const toggleFilter = value => {
    const nextFeatures = selectedFeatures.includes(value)
  ...
  return (
    <section className="rounded-xl bg-white p-4 shadow-sm dark:bg-gray-900">
      <div>
        <h2 className="text-lg font-semibold text-gray-950 dark:text-white">Tất cả sản phẩm</h2>
        <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
          Khám phá các sản phẩm tài khoản số chất lượng, giao hàng nhanh và hỗ trợ 24/7.
        </p>
      </div>
        ...
        <div className="mt-4 flex flex-col gap-3 text-sm sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap items-center gap-2 text-gray-500 dark:text-gray-400">
            ...
            {selectedFeatures.map(value => {
              ...
              return (
                <span key={value} className="rounded-full bg-gray-100 px-3 py-1 font-medium text-gray-700 dark:bg-gray-800 dark:text-gray-200">
                  {filter.value === 'featured' ? 'Nổi bật' : filter.label}
                </span>
              )
            })}
          </div>
...
export default ProductCategoryFiltersSection
