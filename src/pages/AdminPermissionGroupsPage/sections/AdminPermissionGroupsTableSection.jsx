...
import {
  getLocalizedPermissionGroupDescription,
...
function GroupStatusBadge({ isActive }) {
  ...
  return (
    <span className={`admin-permission-groups-status ${isActive ? 'admin-permission-groups-status--active' : 'admin-permission-groups-status--inactive'}`}>
  ...
  return (
    ...
    >
      <Button
        ...
        className="admin-permission-groups-menu-btn"
      />
    </Dropdown>
  )
}

function GroupMobileItem({
  group,
  canEditGroup,
  canDeleteGroup,
  onEditGroup,
...
export default function AdminPermissionGroupsTableSection({
  ...
  const showTotal = (count, range) => t('table.showTotal', {
    rangeStart: range[0],
    rangeEnd: range[1],
    total: count
  })
        ...
        <Table
          ...
          pagination={{
            ...
            showTotal: (count, range) => (
              <span className="text-[var(--admin-text-muted)]">{showTotal(count, range)}</span>
            ),
