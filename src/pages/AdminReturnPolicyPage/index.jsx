...
import {
  getAdminReturnPolicyContent,
  updateAdminReturnPolicyContent
} from '@/services/returnPolicyContentService'
import {
  DEFAULT_RETURN_POLICY_CONTENT,
  DEFAULT_RETURN_POLICY_TRANSLATIONS,
  alignReturnPolicyTranslation,
  normalizeReturnPolicyContent
} from '@/pages/ReturnPolicyPage/returnPolicyContent'
import './AdminReturnPolicyPage.scss'
...
function AdminReturnPolicyPage() {
  ...
  const fetchContent = useCallback(async () => {
    ...
    try {
      const response = await getAdminReturnPolicyContent()
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await updateAdminReturnPolicyContent({
        content,
        translations: {
          en: englishTranslation
        }
  ...
  const renderRefundMethods = root => (
    <Form.List name={fieldName(root, 'refund', 'methods')}>
      {(fields, { add, remove }) => (
        <ListShell title={t('sections.refundMethods')} addText={t('actions.addRefundMethod')} onAdd={() => add({ key: '', method: '', time: '', fee: '', popular: false })}>
          {fields.map(field => (
            <div className="admin-return-policy-list-item" key={field.key}>
              <Row gutter={16}>
                <Col xs={24} md={5}>
  ...
  const renderCategories = root => (
    <Form.List name={fieldName(root, 'categories', 'items')}>
      {(fields, { add, remove }) => (
        <ListShell title={t('sections.categoryItems')} addText={t('actions.addCategory')} onAdd={() => add({ key: '', category: '', returnPeriod: '', conditions: [], specialNotes: '' })}>
          {fields.map(field => (
            <div className="admin-return-policy-list-item" key={field.key}>
              <Row gutter={16}>
                <Col xs={24} md={5}>
  ...
  const renderReasons = root => (
    <Form.List name={fieldName(root, 'reasons', 'items')}>
      {(fields, { add, remove }) => (
        <ListShell title={t('sections.reasonItems')} addText={t('actions.addReason')} onAdd={() => add({ value: '', color: '', label: '' })}>
          {fields.map(field => (
            <div className="admin-return-policy-list-item" key={field.key}>
              <Row gutter={16}>
                <Col xs={24} md={8}>
    ...
    <>
      ...
      <Section title={t('sections.conditions')}>
        <Row gutter={16}>
          ...
          <Col xs={24} md={12}>
            {renderStringList({ root, path: ['conditions', 'acceptedPhysicalItems'], title: t('sections.acceptedPhysical'), addText: t('actions.addItem'), label: t('fields.item') })}
            {renderStringList({ root, path: ['conditions', 'acceptedDigitalItems'], title: t('sections.acceptedDigital'), addText: t('actions.addItem'), label: t('fields.item') })}
          </Col>
          <Col xs={24} md={12}>
            {renderStringList({ root, path: ['conditions', 'rejectedPhysicalItems'], title: t('sections.rejectedPhysical'), addText: t('actions.addItem'), label: t('fields.item') })}
            {renderStringList({ root, path: ['conditions', 'rejectedDigitalItems'], title: t('sections.rejectedDigital'), addText: t('actions.addItem'), label: t('fields.item') })}
          </Col>
        </Row>
      </Section>
      ...
      <Section title={t('sections.support')}>
        <Row gutter={16}>
          <Col xs={24} md={12}>
            ...
            {renderStringList({ root, path: ['support', 'tips'], title: t('sections.supportTips'), addText: t('actions.addTip'), label: t('fields.tip') })}
          </Col>
          <Col xs={24} md={12}>
...
export default AdminReturnPolicyPage
