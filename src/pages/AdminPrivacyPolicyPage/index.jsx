...
import {
  getAdminPrivacyPolicyContent,
  updateAdminPrivacyPolicyContent
} from '@/services/privacyPolicyContentService'
import {
...
} from '@/pages/PrivacyPolicyPage/privacyPolicyContent'
import './AdminPrivacyPolicyPage.scss'
...
function AdminPrivacyPolicyPage() {
  ...
  const fetchContent = useCallback(async () => {
    ...
    try {
      const response = await getAdminPrivacyPolicyContent()
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await updateAdminPrivacyPolicyContent({
        content,
        translations: {
          en: englishTranslation
        }
  ...
  const renderCookies = root => (
    <Form.List name={fieldName(root, 'cookiesSection', 'categories')}>
      {(fields, { add, remove }) => (
        <ListShell title={t('sections.cookieCategories')} addText={t('actions.addCookie')} onAdd={() => add({ key: '', title: '', description: '' })}>
          {fields.map(field => (
            <div className="admin-privacy-policy-list-item" key={field.key}>
              <Row gutter={16}>
                <Col xs={24} md={6}>
    ...
    <>
      ...
      <Section title={t('sections.dataCollection')}>
        <Row gutter={16}>
          ...
          <Col xs={24} md={16}>
            <TextField root={root} path={['dataCollectionSection', 'importantNoteDescription']} label={t('fields.importantNoteDescription')} rows={2} />
          </Col>
        </Row>
