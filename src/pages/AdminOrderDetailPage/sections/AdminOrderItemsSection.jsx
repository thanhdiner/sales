...
import {
  ...
  getAdminOrderDeliveryTypeLabel,
  getAdminOrderItemFallbackIcon,
  getAdminOrderItemKey,
  getAdminOrderItemName,
    ...
    <div className="rounded-xl border border-[var(--admin-border)] bg-[var(--admin-surface)] p-4 shadow-[var(--admin-shadow)] sm:p-5">
      ...
      <div className="space-y-3">
        {orderItems.map(item => {
          ...
          const itemName = getAdminOrderItemName(item, language, item.name || '')
          const deliveryTypeLabel = item.deliveryType ? getAdminOrderDeliveryTypeLabel(item.deliveryType, t) : ''
            ...
            >
              ...
              <div className="shrink-0 text-left sm:text-right">
                ...
                <p className="mt-1 text-sm font-semibold text-[var(--admin-text)]">
                  {formatAdminOrderDetailCurrency(item.price, language)}
                </p>
              </div>
            </div>
          )
