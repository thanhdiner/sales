...
import {
  ...
  getLocalizedAdminAccountFullName,
...
function getAvatarFallback(record, language) {
  const fallbackSource = getLocalizedAdminAccountFullName(record, language, record.fullName || '') || record.username || record.email
...
function getDisplayName(record, language, t) {
  return getLocalizedAdminAccountFullName(record, language, record.fullName || '') || record.username || t('common.unknownAccount')
}

function getRoleLabel(value, roles, language, t) {
  if (value && typeof value === 'object') {
    return getLocalizedAdminRoleLabel(value, language, t('common.unknownRole'))
  }

  const foundRole = roles.find(role => role._id === value)
  return getLocalizedAdminRoleLabel(foundRole, language, t('common.unknownRole'))
}
...
function AccountStatusSelect({ value, record, onChangeStatus }) {
  ...
  return (
    <Select
      ...
      options={getAdminAccountStatusOptions(t)}
      size="small"
    />
  )
}
