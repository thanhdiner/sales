    ...
    <div className="space-y-5">
      ...
      >
        ...
        >
          {field => (
            <>
              <FieldPair path={['steps', field.name, 'title']} label={text('Tiêu đề bước', 'Step title')} />
              <FieldPair path={['steps', field.name, 'content']} label={text('Mô tả bước', 'Step content')} multiline rows={3} />
            </>
          )}
        </GuideList>
      </SettingsPanel>
      ...
      >
        ...
        >
          {field => (
            <>
              <FieldPair path={['detailedSteps', field.name, 'id']} label={text('Mã bước', 'Step label')} />
              <FieldPair path={['detailedSteps', field.name, 'title']} label={text('Tiêu đề', 'Title')} />
              <FieldPair path={['detailedSteps', field.name, 'description']} label={text('Mô tả', 'Description')} multiline rows={3} />
              <LinesPair
                path={['detailedSteps', field.name, 'chips']}
                label={text('Nhãn chip', 'Chips')}
                rows={3}
                hint={text('Mỗi dòng là một chip.', 'Each line is one chip.')}
              />
              <LinesPair
                path={['detailedSteps', field.name, 'checks']}
                label={text('Checklist', 'Checklist')}
                rows={3}
                hint={text('Mỗi dòng là một mục checklist.', 'Each line is one checklist item.')}
              />
              <FieldPair path={['detailedSteps', field.name, 'note']} label={text('Ghi chú', 'Note')} multiline rows={3} />
              <FieldSingle path={['detailedSteps', field.name, 'image']} label={text('URL ảnh', 'Image URL')} />
              <Form.Item name={guidePath(['detailedSteps', field.name, 'reverse'])} valuePropName="checked" className="mb-0">
                <Checkbox className="text-[var(--admin-text-muted)]">{text('Đảo vị trí ảnh/chữ trên desktop', 'Reverse image/text on desktop')}</Checkbox>
              </Form.Item>
            </>
          )}
        </GuideList>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        >
          ...
          >
            {field => (
              <>
                <FieldPair path={['paymentMethods', field.name, 'name']} label={text('Tên phương thức', 'Method name')} />
                <FieldPair path={['paymentMethods', field.name, 'desc']} label={text('Mô tả', 'Description')} multiline rows={3} />
                <LinesPair
                  path={['paymentMethods', field.name, 'badges']}
                  label={text('Badge', 'Badges')}
                  rows={3}
                  hint={text('Mỗi dòng là một badge.', 'Each line is one badge.')}
                />
                <Form.Item name={guidePath(['paymentMethods', field.name, 'popular'])} valuePropName="checked" className="mb-0">
                  <Checkbox className="text-[var(--admin-text-muted)]">{text('Đánh dấu phổ biến', 'Mark as popular')}</Checkbox>
                </Form.Item>
              </>
            )}
          </GuideList>
        ...
        >
          ...
          >
            {field => (
              <>
                <FieldPair path={['faq', field.name, 'question']} label={text('Câu hỏi', 'Question')} />
                <FieldPair path={['faq', field.name, 'answer']} label={text('Câu trả lời', 'Answer')} multiline rows={5} />
              </>
            )}
          </GuideList>
        </SettingsPanel>
