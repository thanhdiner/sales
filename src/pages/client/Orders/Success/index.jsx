...
import { removeManyCartItems } from '@/services/client/commerce/cart'
import { getOrderDetail } from '@/services/client/commerce/order'
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-white px-4 py-12 dark:bg-gray-900">
        ...
        <div className="text-center">
          ...
          <p className="mt-5 text-sm font-medium text-gray-600 dark:text-gray-300">Đang xử lý kết quả thanh toán...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-white px-4 py-12 dark:bg-gray-900">
      ...
      <div className="mx-auto w-full max-w-lg rounded-2xl border border-gray-200 bg-white p-6 text-center shadow-sm dark:border-gray-700 dark:bg-gray-800 md:p-8">
        ...
        <h1 className="text-3xl font-semibold tracking-[-0.03em] text-gray-900 dark:text-white">
          {isSuccess ? (gatewayName ? `Thanh toán ${gatewayName} thành công` : 'Đặt hàng thành công') : 'Thanh toán thất bại'}
        </h1>
        ...
        {isSuccess ? (
          <>
            {orderId && (
              <div className="mt-5 rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-900/30">
                <p className="mb-1 text-sm text-gray-500 dark:text-gray-400">Mã đơn hàng</p>
                <p className="mb-0 text-base font-semibold text-gray-900 dark:text-gray-100">{String(orderId).slice(-8).toUpperCase()}</p>
              </div>
            )}
          ...
          <>
            <p className="mx-auto mt-5 max-w-md text-sm leading-6 text-gray-600 dark:text-gray-300">{getFailureMessage()}</p>
