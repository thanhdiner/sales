...
import { getWishlistInfiniteQueryKey, isWishlistItemInStock } from '../utils/wishlistUtils'
...
export function useWishlistData({ isLoggedIn, queryClient, queryScope, t }) {
  ...
  const visibleWishlists = useMemo(() => wishlistData?.pages?.slice(0, visiblePageCount) || [], [visiblePageCount, wishlistData])
  const wishlistItems = useMemo(() => visibleWishlists.flatMap(page => page?.items || []), [visibleWishlists])
