...
import { getFaqContent } from '@/services/client/content/faq'
...
const FAQ = () => {
  ...
  const content = useMemo(() => normalizeFaqContent(faqPageData, { language, phone: supportPhone }), [faqPageData, language, supportPhone])
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12">
      ...
      <div className="mb-9 text-center">
        <p className="mb-3 text-sm font-semibold uppercase tracking-[0.16em] text-gray-500 dark:text-gray-400">{content.page.eyebrow}</p>

        <Title level={2} className="!mb-0 !text-3xl !font-semibold !tracking-[-0.03em] !text-gray-900 dark:!text-white">
          {content.page.title}
        </Title>

        <p className="mx-auto mt-3 max-w-2xl text-base leading-7 text-gray-600 dark:text-gray-300">{content.page.description}</p>
      </div>
      ...
      >
        {faqData.map((faq, idx) => (
          ...
          >
            <p className="mb-0 text-sm leading-6 text-gray-600 dark:text-gray-300">{faq.answer}</p>
          </Panel>
        ))}
      </Collapse>
