...
import { removeCompareLocal, clearCompareLocal } from '@/stores/client/compare'
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-20 px-4">
        <div className="container mx-auto text-center max-w-lg">
          <div className="bg-white dark:bg-gray-800 p-12 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700">
            ...
            <Link
              to="/products"
              className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-8 py-3.5 rounded-2xl font-bold transition-all shadow-lg active:scale-95"
            >
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <Link
                to="/products"
                className="flex items-center gap-2 px-6 py-2.5 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 rounded-xl font-bold transition-all shadow-sm"
              >
                Tiếp tục mua hàng
              </Link>
            </div>
          ...
          <table className="w-full border-collapse">
              ...
              <tr className="border-b border-gray-100 dark:border-gray-700">
                ...
                {compareItems.map(item => (
                  <th key={item.productId} className="p-6 min-w-[260px] max-w-[300px] align-top relative group">
                    ...
                    <div className="text-xl font-extrabold text-red-500 mb-4 tracking-tighter">{formatPrice(item.price)}</div>
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-700">
              <tr>
                ...
                {Array.from({ length: Math.max(0, 4 - compareItems.length) }).map((_, i) => (
                  <td key={`empty-rate-${i}`} className="p-5">
                    -
                  </td>
                ))}
              </tr>
              ...
              <tr>
                ...
                {Array.from({ length: Math.max(0, 4 - compareItems.length) }).map((_, i) => (
                  <td key={`empty-disc-${i}`} className="p-5">
                    -
                  </td>
                ))}
              </tr>
              ...
              <tr>
                ...
                {Array.from({ length: Math.max(0, 4 - compareItems.length) }).map((_, i) => (
                  <td key={`empty-saved-${i}`} className="p-5">
                    -
                  </td>
                ))}
              </tr>
              ...
              <tr>
                ...
                {compareItems.map(item => (
                  <td key={item.productId} className="p-5">
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-2.5 h-2.5 rounded-full ${item.inStock ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]' : 'bg-red-500'}`}
                      />
                ...
                {Array.from({ length: Math.max(0, 4 - compareItems.length) }).map((_, i) => (
                  <td key={`empty-stock-${i}`} className="p-5">
                    -
                  </td>
                ))}
              </tr>
              ...
              <tr>
                <td className="p-5 font-bold text-gray-700 dark:text-gray-300 bg-gray-50/50 dark:bg-gray-900/10 align-top">
                  Mô tả sản phẩm
                </td>
                {compareItems.map(item => (
                ...
                {Array.from({ length: Math.max(0, 4 - compareItems.length) }).map((_, i) => (
                  <td key={`empty-desc-${i}`} className="p-5">
                    -
                  </td>
                ))}
              </tr>
        ...
        <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="p-8 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[2rem] text-white shadow-xl shadow-blue-500/20">
            ...
            <p className="text-blue-50 opacity-90 leading-relaxed">
              Khi so sánh các sản phẩm ảo như tài khoản, hãy chú ý đến thời hạn sử dụng và chế độ bảo hành. SmartMall luôn cam kết bảo hành
              1:1 cho mọi sản phẩm!
            </p>
          </div>
          ...
          <div className="p-8 bg-white dark:bg-gray-800 rounded-[2rem] border border-gray-100 dark:border-gray-700 shadow-xl">
            ...
            <p className="text-gray-500 dark:text-gray-400 leading-relaxed">
              Sản phẩm nào có lượt mua nhiều và đánh giá cao thường là các gói được cộng đồng yêu thích nhất. Hãy cân nhắc các gói{' '}
              <b>Top Deal</b> để có giá tốt nhất!
            </p>
          </div>
        </div>
      </div>
