...
function AdminGameNewsPage() {
  const { t } = useTranslation('adminGameNews')
  ...
  const fetchContent = useCallback(async () => {
    ...
    try {
      const response = await getAdminGameNewsContent()
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await updateAdminGameNewsContent({
        ...(values.content || {}),
        translations: {
      ...
      queryClient.invalidateQueries({ queryKey: ['gameNewsContent'] })
  ...
  const Section = ({ title, description, children }) => (
    <section className="admin-game-news-section">
      <div className="admin-game-news-section__header">
        <h3>{title}</h3>
  ...
  return (
    <div className="admin-game-news-page">
      <SEO title={t('seo.title')} noIndex />

      <div className="admin-game-news-card">
        <div className="admin-game-news-header">
          <div>
          ...
          <Space wrap>
            <Button icon={<EyeOutlined />} onClick={() => window.open('/game-news', '_blank', 'noopener,noreferrer')}>
              {t('actions.preview')}
            </Button>
        ...
        <Spin spinning={loading}>
          <Form form={form} layout="vertical" onFinish={handleSubmit} className="admin-game-news-form">
            <Tabs
