...
import { generateAdminProductContent } from '@/services/adminProductService'
...
const renderPreviewValue = value => {
  if (Array.isArray(value)) {
    return (
      <ul>
        {value.map((item, index) => <li key={`${item}-${index}`}>{item}</li>)}
      </ul>
    )
  }
...
export default function ProductAIAssistant({ classNamePrefix, form, language = 'vi', size = 'small', target }) {
  ...
  const requestContent = async nextAction => {
    ...
    try {
      const response = await generateAdminProductContent({
        action: nextAction,
        target,
        language,
  ...
  return (
    <div className={`${classNamePrefix}__ai-assistant`}>
      ...
      <Modal
        ...
        width={760}
      >
        <div className={`${classNamePrefix}__ai-preview`}>
