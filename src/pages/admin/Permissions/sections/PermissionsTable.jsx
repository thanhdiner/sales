...
export default function PermissionsTable({
  ...
  const renderActions = record => (
    <Space size="small" className="admin-permissions-actions">
      ...
      {!canEditPermission && !canDeletePermission && <span className="text-[var(--admin-text-subtle)]">{t('common.noActions')}</span>}
    </Space>
  )
  ...
  const columns = [
    {
      ...
      width: '18%',
      render: name => (
        <span className="admin-permissions-code-text font-mono text-sm font-medium text-[var(--admin-text-muted)]">{name}</span>
      )
    },
    {
