...
const secondaryButtonClass =
  'rounded-lg !h-9 !px-3 !border-[var(--admin-border)] !bg-[var(--admin-surface)] !text-[var(--admin-text-muted)] hover:!border-[var(--admin-border-strong)] hover:!bg-[var(--admin-surface-2)] hover:!text-[var(--admin-text)]'
const dangerButtonClass =
  'rounded-lg !h-9 !px-3 !border-red-200 !bg-red-50 !text-red-600 hover:!border-red-300 hover:!bg-red-100 dark:!border-red-900/50 dark:!bg-red-950/30 dark:!text-red-300'
...
export default function NotificationsFilters({
  ...
  return (
    <section className="admin-notifications-filters rounded-xl border bg-[var(--admin-surface)] p-3 shadow-[var(--admin-shadow)]">
      ...
      <div className="admin-notifications-filters__controls mt-2 grid gap-2 pt-2 lg:grid-cols-[minmax(220px,1.4fr)_minmax(160px,0.8fr)_minmax(150px,0.7fr)_minmax(150px,0.7fr)_minmax(230px,1fr)_auto]">
        <SearchInput
          value={filters.search}
          onChange={event => updateFilter('search', event.target.value)}
          onClear={() => updateFilter('search', '')}
          placeholder={t('filters.searchPlaceholder')}
          className="!h-9 rounded-lg border-[var(--admin-border)] bg-[var(--admin-surface-2)] text-[var(--admin-text)]"
        />

        <Select
          value={filters.type}
          onChange={value => updateFilter('type', value)}
          className="h-9 [&_.ant-select-selector]:!h-9 [&_.ant-select-selector]:!rounded-lg [&_.ant-select-selector]:!border-[var(--admin-border)] [&_.ant-select-selector]:!bg-[var(--admin-surface-2)] [&_.ant-select-selection-item]:!text-[var(--admin-text)] [&_.ant-select-selection-item]:!leading-9"
          options={[
        ...
        <Button icon={<X className="h-4 w-4" />} onClick={onClearFilters} className={secondaryButtonClass}>
          {t('filters.clear')}
        </Button>
      </div>

      {selectedCount > 0 && (
        <div className="mt-3 flex flex-col gap-3 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-surface-2)] p-3 sm:flex-row sm:items-center sm:justify-between">
