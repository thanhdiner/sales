  ...
  return (
    <div className="admin-page-builder">
      ...
      <div className="admin-page-builder__topbar">
        <Button icon={<ArrowLeft size={16} />} onClick={() => navigate('/admin/blog')}>
          Back
        </Button>
        <div>
          <h1>{config.title}</h1>
        ...
        <Space>
          <Button icon={<Eye size={16} />} onClick={() => window.open(config.previewUrl, '_blank', 'noopener,noreferrer')}>
            Preview
          </Button>
          ...
          <Button type="primary" icon={<Send size={16} />} loading={publishing} onClick={handlePublish}>
            Publish
          </Button>
        </Space>
      </div>
      ...
      {loading ? (
        <div className="admin-page-builder__loading">
          <Spin />
        </div>
      ) : (
                  ...
                  children: (
                    <div className="admin-page-builder__blocks">
                      {config.sectionTypes.map(meta => (
                        <Button key={meta.type} block icon={<Plus size={15} />} onClick={() => addSection(meta)}>
                          {meta.label}
                        </Button>
                      ))}
                    </div>
                    ...
                    <div className="admin-page-builder__navigator">
                      {sections.map(section => (
                        <Button
                          key={section.id}
                          block
                          type={section.id === selectedId ? 'primary' : 'default'}
                          onClick={() => setSelectedId(section.id)}
                        >
                          {getSectionMeta(section.type, config.sectionTypes).label}
                        </Button>
                      ))}
                    </div>
                  )
  ...
  return (
    ...
    >
      <div className="admin-page-builder__section-head">
        <button type="button" className="admin-page-builder__drag" {...attributes} {...listeners}>
...
function SectionPreview({ section }) {
  ...
  if (section.type === 'hero')
    return (
      <div className="admin-page-builder__hero">
  ...
  if (section.type === 'cta')
    return (
      <div className="admin-page-builder__cta">
        <strong>{settings.title}</strong>
        <p>{settings.description}</p>
        <Space>
          <Button>{settings.primaryText}</Button>
          <Button>{settings.secondaryText}</Button>
        </Space>
      </div>
    )
  return (
    <div className="admin-page-builder__mock-grid">
      <span />
      <span />
      <span />
      <span />
...
function PageSettings({ value, onChange }) {
  return (
    <Form layout="vertical" initialValues={value} onValuesChange={(_, values) => onChange(values)}>
      ...
      <Form.Item name={['seo', 'thumbnail']} label="Thumbnail URL">
        <Input />
      </Form.Item>
    </Form>
  )
...
function SectionSettings({ form, section, onChange }) {
  return (
    <Form form={form} layout="vertical" onValuesChange={(_, values) => onChange(values)}>
      ...
      {['featured_posts', 'popular_posts', 'tag_cloud'].includes(section.type) ? (
        <ListFields showHint={section.type === 'popular_posts'} />
      ) : null}
      {section.type === 'latest_articles' ? <LatestFields /> : null}
      {section.type === 'category_tabs' ? (
        <Form.Item name="label" label="Label">
          <Input />
        </Form.Item>
      ) : null}
      {section.type === 'cta' ? <CtaFields /> : null}
      {['post_header', 'post_content', 'table_of_contents', 'author_box', 'related_products', 'related_posts', 'tags', 'comments'].includes(
        section.type
      ) ? (
...
function HeroFields() {
  return (
    <>
      ...
      <Form.Item name="showStats" label="Show stats" valuePropName="checked">
        <Switch />
      </Form.Item>
    </>
  )
...
function LatestFields() {
  return (
    <>
      ...
      <Form.Item name="limit" label="Initial limit">
        <InputNumber min={1} max={24} className="w-full" />
      </Form.Item>
      <Form.Item name="showLoadMore" label="Show load more" valuePropName="checked">
        <Switch />
      </Form.Item>
    </>
  )
...
function CtaFields() {
  return (
    <>
      ...
      <Form.Item name="primaryUrl" label="Primary URL">
        <Input />
      </Form.Item>
      <Form.Item name="secondaryText" label="Secondary text">
        <Input />
      </Form.Item>
      <Form.Item name="secondaryUrl" label="Secondary URL">
        <Input />
      </Form.Item>
    </>
  )
...
function DetailFields({ section }) {
  return (
    <>
      {['table_of_contents', 'author_box', 'related_products', 'related_posts', 'tags', 'comments'].includes(section.type) ? (
        <Form.Item name="title" label="Title">
          <Input />
        </Form.Item>
      ) : null}
      {['related_products', 'related_posts'].includes(section.type) ? (
        <Form.Item name="limit" label="Limit">
          <InputNumber min={1} max={12} className="w-full" />
        </Form.Item>
      ) : null}
      {section.type === 'post_header' ? (
        <Form.Item name="showShare" label="Show share" valuePropName="checked">
          <Switch />
        </Form.Item>
      ) : null}
    </>
