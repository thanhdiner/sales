...
import { getRoles } from '@/services/admin/rbac/role'
...
import {
  ACCOUNT_FORM_INITIAL_VALUES,
  buildAccountFormData,
  getAccountAvatarFileList,
  getAccountRoleId
} from '../utils'
...
export default function useAccounts() {
  ...
  const handleOpenCreate = useCallback(() => {
    ...
    if (roles.length) {
      ...
      form.setFieldsValue(ACCOUNT_FORM_INITIAL_VALUES)
    }
  ...
  const handleOpenEdit = useCallback((account) => {
    ...
    form.setFieldsValue({
      ...
      role_id: getAccountRoleId(account),
      avatarUrl: getAccountAvatarFileList(account)
    })
    setModalOpen(true)
  }, [form])
  ...
  const handleSave = useCallback(async () => {
    try {
      ...
      if (editing) {
        const response = await updateAccount(editing._id, formData)
