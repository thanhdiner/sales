...
export function getAccountStatusOptions(t) {
  return ACCOUNT_STATUS_OPTIONS.map(option => ({
    ...option,
    label: t(ACCOUNT_STATUS_LABEL_KEYS[option.value] || option.label)
  }))
}

export const ACCOUNT_FORM_INITIAL_VALUES = {
  status: 'active',
  avatarUrl: [],
  translations: {
    en: {
...
export const getLocalizedAccountFullName = (account, language, fallback = '') => {
...
export const getAccountRoleId = account => (
  typeof account?.role_id === 'object' && account?.role_id?._id
    ? account.role_id._id
    : account?.role_id
)

export const getAccountAvatarFileList = account => (
  account?.avatarUrl
    ? [
        {
          uid: account._id,
...
export const ADMIN_ACCOUNT_FORM_INITIAL_VALUES = ACCOUNT_FORM_INITIAL_VALUES
export const getAdminAccountStatusOptions = getAccountStatusOptions
export const getLocalizedAdminRoleLabel = getLocalizedRoleLabel
