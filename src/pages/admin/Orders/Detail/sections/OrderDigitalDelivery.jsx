...
import { formatOrderDetailDate, getOrderItemName } from '../utils'
...
const getDigitalDeliveries = (orderItems, language) =>
  (orderItems || [])
    .flatMap(item => (item.digitalDeliveries || []).map((delivery, index) => ({
      ...delivery,
      productName: getOrderItemName(item, language, item.name || ''),
      index: index + 1
    })))
...
export default function OrderDigitalDelivery({ orderItems = [], paymentStatus }) {
        ...
        <div className="mb-4 rounded-lg border border-[color-mix(in_srgb,#f59e0b_30%,var(--admin-border))] bg-[color-mix(in_srgb,#f59e0b_12%,var(--admin-surface-2))] p-3 text-sm text-[#b45309] dark:text-[#fbbf24]">
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="mb-0">
                {pendingInstantItems.map(item => getOrderItemName(item, language, item.name || '')).join(', ')}
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="mb-3 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                ...
                {delivery.deliveredAt && (
                  <span className="text-xs text-[var(--admin-text-muted)]">
                    {formatOrderDetailDate(delivery.deliveredAt, language)}
                  </span>
                )}
              </div>
