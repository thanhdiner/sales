...
export default function OrdersFilters({ keyword, status, onClearFilters, onKeywordChange, onStatusChange }) {
  ...
  return (
    <div className="admin-orders-filters mb-4 rounded border border-[var(--admin-border)] bg-[var(--admin-surface)] p-3 sm:p-4">
      ...
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_220px_auto] lg:items-center">
        ...
        <Select
          ...
          className="admin-orders-status-select w-full rounded font-sans text-[var(--admin-text)]"
          popupClassName="admin-orders-select-dropdown"
          placeholder={t('filters.statusPlaceholder')}
          options={statusOptions}
        />

        <Button className="admin-orders-filter-clear" icon={<ReloadOutlined />} onClick={onClearFilters}>
          {t('filters.clear')}
        </Button>
      </div>
    </div>
