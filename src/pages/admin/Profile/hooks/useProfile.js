...
import { updateProfile } from '@/stores/admin/adminUser'
import { updateAvatar, updateProfile as updateAccountProfile } from '@/services/admin/users/account'
...
export default function useProfile({ language = 'vi', t = key => key } = {}) {
  ...
  const handleFileChange = async event => {
    ...
    try {
      ...
      const response = await updateAvatar(profile._id, formData)
  ...
  const handleSave = async values => {
    ...
    try {
      const response = await updateAccountProfile(profile._id, { fullName: values.fullName })
      dispatch(updateProfile(response.data))
