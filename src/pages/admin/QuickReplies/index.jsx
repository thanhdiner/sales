...
import {
  ...
  updateQuickReply
} from '@/services/admin/agent/quickReply'
import './index.scss'
...
function QuickRepliesTable({ categoryOptions, data, loading, pagination, onChangePagination, onEdit, onDelete, onToggleStatus }) {
  const columns = [
    {
      title: 'Template name',
      dataIndex: 'title',
...
function QuickReplyFormModal({ categoryOptions, open, editing, form, submitLoading, onClose, onSubmit, onInsertVariable }) {
  ...
  return (
    ...
    >
      <div ref={contentRef}>
        <Form form={form} layout="vertical" initialValues={FORM_INITIAL_VALUES} onFinish={onSubmit} className="admin-quick-replies-form">
          <Form.Item
            label="Template name"
            name="title"
            rules={[
          ...
          <div className="admin-quick-replies-variable-row">
            {VARIABLE_OPTIONS.map(variable => (
              <button key={variable} type="button" className="admin-quick-replies-variable-chip" onClick={() => onInsertVariable(variable)}>
                {`{{${variable}}}`}
              </button>
            ))}
          </div>
...
function ManageCategoriesModal({
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'isActive',
      width: 92,
      render: (_, record) => <Switch size="small" checked={record.isActive} onChange={checked => onToggleStatus(record, checked)} />
    },
    {
      title: 'Actions',
      key: 'actions',
  ...
  return (
    ...
    >
      <div ref={contentRef} className="admin-quick-replies-categories">
        ...
        >
          <Form form={form} layout="vertical" initialValues={CATEGORY_FORM_INITIAL_VALUES} onFinish={onSubmit}>
            ...
            <div className="admin-quick-replies-category-form-actions">
              <Button type="primary" htmlType="submit" loading={submitLoading} className="admin-quick-replies-primary-btn">
...
export default function QuickReplies() {
  ...
  const memoizedStats = useMemo(
    () => ({
      ...stats,
      categories: categories.length > 0 ? categories.filter(category => category.isActive).length : stats.categories
    }),
    [categories, stats]
  )
