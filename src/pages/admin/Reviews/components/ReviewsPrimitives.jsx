...
export function ReviewsStarDisplay({ value, size = 14 }) {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map(star => (
        <Star
          ...
          size={size}
          strokeWidth={1.8}
          className={star <= Math.round(Number(value) || 0) ? 'fill-amber-400 text-amber-400' : 'text-[var(--admin-border-strong)]'}
        />
      ))}
    </div>
  )
...
export function ReviewsAvatar({ user }) {
...
export function ReviewsRatingBadge({ rating }) {
  return (
