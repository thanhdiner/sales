...
export default function ReviewsList({ loading, reviews, onReply, onDeleteReply, onHide, onDelete }) {
  if (loading) {
  ...
  return (
    <div className="space-y-4">
      {reviews.map(review => (
        <ReviewRow key={review._id} review={review} onReply={onReply} onDeleteReply={onDeleteReply} onHide={onHide} onDelete={onDelete} />
      ))}
    </div>
  )
}
