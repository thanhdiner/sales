...
export default function ProductInfo({ product, pricing }) {
  ...
  return (
    <div className="flex flex-1 flex-col gap-1.5 px-3 pb-2 pt-2.5 sm:gap-2 sm:px-4 sm:pb-2.5 sm:pt-3">
      <h3 className="min-h-[38px] text-[13px] font-semibold leading-5 text-slate-900 transition-colors duration-200 group-hover:text-blue-600 sm:min-h-[42px] sm:text-sm sm:leading-5 dark:text-[#f4f6f8] dark:group-hover:text-green-300">
        <span className="line-clamp-2">{product.title}</span>
      </h3>

      <ProductRating rate={product.rate} reviewCount={product.reviewCount ?? product.reviewsCount ?? product.totalReviews} />

      <div className="flex min-h-[24px] flex-wrap items-center gap-1.5 text-[10.5px] text-slate-500 sm:gap-2 sm:text-[11px] dark:text-[#a8b0ba]">
