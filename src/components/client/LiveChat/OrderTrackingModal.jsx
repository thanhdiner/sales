...
import { trackOrder } from '@/services/client/commerce/order'

const STATUS_CONFIG = {
  pending: { labelKey: 'orderTracking.status.pending', color: 'text-amber-600 bg-amber-50 dark:text-amber-400 dark:bg-amber-500/10', icon: Clock },
  confirmed: { labelKey: 'orderTracking.status.confirmed', color: 'text-blue-600 bg-blue-50 dark:text-blue-400 dark:bg-blue-500/10', icon: CheckCircle2 },
  shipping: { labelKey: 'orderTracking.status.shipping', color: 'text-indigo-600 bg-indigo-50 dark:text-indigo-400 dark:bg-indigo-500/10', icon: Truck },
  completed: { labelKey: 'orderTracking.status.completed', color: 'text-emerald-600 bg-emerald-50 dark:text-emerald-400 dark:bg-emerald-500/10', icon: CheckCircle2 },
  cancelled: { labelKey: 'orderTracking.status.cancelled', color: 'text-red-600 bg-red-50 dark:text-red-400 dark:bg-red-500/10', icon: XCircle },
}

const PAYMENT_STATUS_CONFIG = {
  pending: { labelKey: 'orderTracking.paymentStatus.pending', color: 'text-amber-600' },
  paid: { labelKey: 'orderTracking.paymentStatus.paid', color: 'text-emerald-600' },
  failed: { labelKey: 'orderTracking.paymentStatus.failed', color: 'text-red-600' },
}
...
const getValidationError = ({ orderCode, phone }, t) => {
  if (!orderCode || !phone) {
    return t('orderTracking.validation.required')
  }

  if (!PHONE_REGEX.test(phone)) {
    return t('orderTracking.validation.invalidPhone')
  }
...
export default function OrderTrackingModal({
  ...
  const performSearch = useCallback(async ({ nextOrderCode, nextPhone }) => {
    ...
    const validationError = getValidationError({ orderCode: normalizedOrderCode, phone: normalizedPhone }, t)
    ...
    try {
      ...
      setError(err.message || t('orderTracking.errors.notFound'))
    } finally {
      setLoading(false)
    }
  }, [t])
            ...
            >
              <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4 dark:border-gray-800">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-lg font-bold text-gray-900 dark:text-white">{t('orderTracking.title')}</h2>
                </div>
                <button
                  type="button"
                  onClick={handleClose}
                  ...
                  >
                    <div className="flex items-center justify-between border-b border-gray-200 bg-gray-50 px-5 py-3 dark:border-gray-700 dark:bg-gray-800/50">
                      ...
                      <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${statusCfg.color}`}>
                        ...
                        {statusCfg.labelKey ? t(statusCfg.labelKey) : result.status}
                      </span>
                    </div>

                    <div className="space-y-3 p-5">
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-500 dark:text-gray-400">{t('orderTracking.result.orderDate')}</span>
                      ...
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-500 dark:text-gray-400">{t('orderTracking.result.itemCountLabel')}</span>
                        <span className="font-medium text-gray-900 dark:text-white">
                          {t('orderTracking.result.itemCount', { count: result.itemCount })}
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-500 dark:text-gray-400">{t('orderTracking.result.payment')}</span>
                        <span className={`font-medium ${paymentCfg.color}`}>
                          {paymentCfg.labelKey ? t(paymentCfg.labelKey) : result.paymentStatus}
                        </span>
                      </div>
                      <div className="flex justify-between border-t border-gray-100 pt-3 text-sm dark:border-gray-700">
                        <span className="text-gray-500 dark:text-gray-400">{t('orderTracking.result.total')}</span>
                        <span className="font-bold text-blue-600 dark:text-blue-400">
                          {new Intl.NumberFormat(locale, { style: 'currency', currency: 'VND' }).format(result.total || 0)}
                        </span>
                      </div>
                    </div>
                  </motion.div>
